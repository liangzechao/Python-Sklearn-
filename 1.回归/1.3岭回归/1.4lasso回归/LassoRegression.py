import numpy as np
from sklearn.linear_model import Lasso              # 通过sklearn.linermodel加载Lasso回归方法
from sklearn import cross_validation                # 加载交叉验证模块
import matplotlib.pyplot as plt
from sklearn.preprocessing import PolynomialFeatures

# a=pd.read_csv('data.csv')                           # 使用pandas的方法从csv文件中加载数据
data = np.loadtxt('D:/Workplace/gitbook_ML/1.回归/1.4lasso回归/data.csv', delimiter=",",skiprows=1)
...
las =Lasso(alpha=1.0,fit_intercept=True)            # 创建岭回归实例
las.fit(train_set_X,train_set_y)                    # 调用fit函数使用训练集训练回归器
las.score(test_set_X,test_set_y)                    # 利用测试集计算回归曲线的拟合优度，clf.score返回值为0.7375拟合优度，


start =200                                          # 画一段200到300范围内的拟合曲线
end =300
y_pre =las.predict(X)                               # 调用predict函数的拟合值
