import pandas as pd
import numpy as np
from sklearn import svm                         #sklearn下svm：SVM算法
from sklearn import cross_validation            # sklearn下cross_validation：交叉验证

#parse_dates=第0列解析为日期， index_col= 用作行索引的列编号）
data=pd.read_csv('D:/Workplace/sklearnML/stock/000777.csv',encoding='gbk',parse_dates=[0],index_col=0)
...
for i in range(0,data.shape[0]-dayfeature):
    #/将数据中的“收盘价”“最高价”“开盘价”“成交量”存入x数组中,u:unicode编码, reshape:转换成1行，featurenum列
    x[i,0:featurenum]=np.array(data[i:i+dayfeature][[u'收盘价',u'最高价',u'最低价',u'开盘价',u'成交量']]).reshape((1,featurenum))
    x[i,featurenum]=data.ix[i+dayfeature][u'开盘价']                                #最后一列记录当日的开盘价              ix :索引
...
result = []
for i in range(5):
    x_train, x_test, y_train, y_test = cross_validation.train_test_split(x, y, test_size = 0.2)     #x和y的验证集和测试集，切分80 - 20 % 的测试集
