...
X=real_X[:,0:1]                                             # 取starttime训练

db=skc.DBSCAN(eps=0.01,min_samples=20).fit(X)               # 调用 DBSCAN 方法进行训练，半径0.01，邻域内的样本点数20
...
for i in range(n_clusters_):
    print('Cluster ',i,':')                                                     # 簇号
    print(list(X[labels == i].flatten()))                                       # 簇的数据
     
plt.hist(X,24)                                  # 直方图
plt.xticks(np.arange(0, 24, 1))                 # 设置横坐标刻度
plt.xlabel('starttime')                         # 设置坐标轴名称
plt.ylabel('student num')
plt.show() 
